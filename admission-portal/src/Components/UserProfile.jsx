...
const UserProfile = () => {
  ...
  const handleNewPassSave = () => {
    ...
    const storedId = localStorage.getItem('userId');
    axios
      .put(`https://8080-deeefebadfbfcffebbdadbedabeaeaadbdbabf.project.examly.io/ap/students/${storedId}`, { ...form, password: newPassword })
      .then((response) => {
        setValue(response.data);
        setShowNewPasswordModal(false);
        setShowModal(false);
  ...
  const handleUpdate = (event) => {
    ...
    const storedId = localStorage.getItem('userId');
    axios
      .put(`https://8080-deeefebadfbfcffebbdadbedabeaeaadbdbabf.project.examly.io/ap/students/${storedId}`, form)
      .then((response) => {
        setValue(response.data);
        setShowModal(false);
        setPhoneNumberError(false);
