...
const UserProfile = () => {
  ...
  const getSingleUserData = (storedId) => {
    axios
      .get(`https://8080-faffbbafcebafebbdadbedabeaeaadbdbabf.project.examly.io/ap/students/${storedId}`)
      .then((response) => {
  ...
  const handleNewPassSave = () => {
    ...
    const storedId = localStorage.getItem('userId');
    axios
      .put(`https://8080-faffbbafcebafebbdadbedabeaeaadbdbabf.project.examly.io/ap/students/${storedId}`, { ...form, password: newPassword })
      .then((response) => {
        setValue(response.data);
        setShowNewPasswordModal(false);
        setShowModal(false);
  ...
  const handleUpdate = (event) => {
    ...
    const storedId = localStorage.getItem('userId');
    axios
      .put(`https://8080-faffbbafcebafebbdadbedabeaeaadbdbabf.project.examly.io/ap/students/${storedId}`, form)
      .then((response) => {
        setValue(response.data);
        setShowModal(false);
        setPhoneNumberError(false);
